...
#tcx .pc-canvas{display:block;width:100%;height:clamp(180px,23vh,224px)}
...
@media(max-width:680px){
  #tcx .pc-canvas{height:clamp(168px,26vh,214px)}
...
function buildShell(h: HTMLElement) {
  ...
  h.querySelector('#pcCopilot')!.addEventListener('click', e => {
    ...
    if (act === 'draft' && s && s.feasible) {
      ...
      const stage = document.getElementById('tcxStage'); if (stage) { stage.classList.add('pc-flash'); stage.scrollIntoView({ behavior: 'smooth', block: 'center' }); setTimeout(() => stage.classList.remove('pc-flash'), 1400); }
...
function ensureCanvas() {
  ...
  const pos = (ev: PointerEvent) => { const r = c.getBoundingClientRect(); hoverX = clamp((ev.clientX - r.left) / r.width, 0, 1); const tip = host!.querySelector('#pcTip') as HTMLElement, f = floorAt(hoverX); if (tip && DATA) { const clr = Math.round(f.cm - DATA.draftCm), m = clr >= DATA.reserveCm ? '✅' : clr >= 0 ? '⚠️' : '⛔'; tip.innerHTML = `<b>${E(f.sec || f.group)}</b> · ${m2(f.cm / 100)} · ${m} ${clr >= 0 ? clr + ' cm Reserve' : Math.abs(clr) + ' cm zu flach'}`; tip.style.opacity = '1'; } if (reduceMotion()) draw(0); };
...
function draw(t: number) {
  ...
  ctx.lineWidth = DPR * 3.2; ctx.lineJoin = 'round'; ctx.shadowBlur = DPR * 9;
  ...
  const boatX = hoverX >= 0 ? hoverX : (tSec / 16) % 1, bx = boatX * W, bob = Math.sin(t * 1.8) * 3 * DPR, bsurf = surfY + bob, keelY = draftY + bob;
